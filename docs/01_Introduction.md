...
Note: The repository contains modular services for creating and managing an embedding index, extracting features with CLIP-compatible encoders, a responsive web UI for querying results, and utilities for preprocessing and storing embeddings. The primary components are encapsulated in the `semantixel/` package.

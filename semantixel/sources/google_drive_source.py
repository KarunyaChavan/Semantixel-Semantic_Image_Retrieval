"""Google Drive OAuth source for listing and fetching remote images."""
...
class GoogleDriveSource:
    """OAuth2-based Google Drive integration for image retrieval.

    Handles the full OAuth flow (authorisation URL → exchange → token
    storage) and provides methods to list images in configured folders
    and fetch their bytes.

    Attributes:
        SOURCE_NAME: Source tag ``"gdrive"`` used in media identifiers.
    """
    ...
    @property
    def settings(self):
        """Shortcut to ``config.google_drive``."""
        return config.google_drive

    def is_enabled(self) -> bool:
        """Whether the Drive integration is enabled in config."""
        return self.settings.enabled

    def is_configured(self) -> bool:
        """Whether config has a client-secret file path set."""
        return self.is_enabled() and bool(self.settings.client_secret_file)

    def dependencies_available(self) -> bool:
        """Whether ``google_auth_oauthlib`` is installed."""
        try:
    ...
    def get_authorization_url(self) -> Dict[str, str]:
        ...
        Returns:
            A dict with ``authorization_url`` and ``state``.

        Raises:
            RuntimeError: If dependencies or configuration are missing.
        """
        import secrets
    ...
    def exchange_code(self, code: str, state: Optional[str]) -> None:
        ...
        Persists the credentials to the token file.

        Args:
            code: The authorisation code from Google's redirect.
            state: The original state parameter for CSRF protection.
        """
        if not self.dependencies_available():
            raise RuntimeError("google-auth-oauthlib is not installed.")
    ...
    def get_status(self) -> Dict[str, Any]:
        """Return a summary of the current Drive integration status."""
        token_exists = (
            os.path.exists(self._token_path()) if self.settings.token_file else False
        )
        return {
    ...
    def list_media(self) -> List[MediaDescriptor]:
        """List image files in configured Google Drive folders.

        Returns:
            List of :class:`MediaDescriptor` objects.
        """
        ...
        params = {
            ...
            "includeItemsFromAllDrives": str(
                self.settings.include_shared_drives
            ).lower(),
        }
        ...
        while True:
            ...
            payload = self._request_json(
                session, "%s/files" % self.DRIVE_API_BASE, params=params
            )
            for file_data in payload.get("files", []):
                display_path = "Google Drive/%s" % file_data["name"]
        ...
        logger.info("Loaded %d Google Drive media items", len(items))
        return items

    def fetch_image(self, locator: str) -> Image.Image:
        """Download a Drive file and return as a PIL Image.

        Args:
            locator: The Google Drive file ID.
    ...
    def fetch_bytes(self, locator: str) -> Tuple[bytes, str, str]:
        ...
        Returns:
            A tuple of ``(content_bytes, mime_type, file_name)``.
        """
        session = self._authorized_session()
        metadata = self._request_json(
            session,
            "%s/files/%s" % (self.DRIVE_API_BASE, locator),
            params={"fields": "id,name,mimeType"},
        )
        response = session.get(
            "%s/%s" % (self.DRIVE_UPLOAD_API_BASE, locator),
            params={"alt": "media"},
            timeout=120,
        )
    ...
    def get_credentials(self):
        """Return valid OAuth credentials, or ``None``."""
        ...
        try:
            ...
            logger.warning("Failed to load Google Drive credentials: %s", exc)
            return None
    ...
    def _mime_query(self) -> str:
        ...
        if not mime_types:
            return ""
        return "(" + " or ".join("mimeType='%s'" % mime for mime in mime_types) + ")"
    ...
    def _folder_query(self) -> str:
        ...
        return (
            "("
            + " or ".join("'%s' in parents" % fid for fid in self.settings.folder_ids)
            + ")"
        )
    ...
    @staticmethod
    def _request_json(session, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
